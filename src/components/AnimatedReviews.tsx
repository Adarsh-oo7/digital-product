      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          {[...reviews, ...reviews].map((review, index) => (
            <div key={index} className="min-w-[380px] bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
              <p className="text-sm text-gray-700 mb-6 leading-relaxed">"{review.content}"</p>
              ...
              <div className="flex items-center gap-4">
                {review.image && (
                  <img src={review.image} alt={review.fullName} className="w-12 h-12 rounded-full object-cover" />
                )}
                <div>
                  <p className="font-semibold text-gray-900">{review.fullName}</p>
                  <p className="text-sm text-gray-500">{review.businessName} • {review.city}</p>
                  {review.website && (
                    <a href={review.website} target="_blank" className="text-sm text-blue-600 hover:underline">
                      Visit Website →
                    </a>
                  )}
                </div>
